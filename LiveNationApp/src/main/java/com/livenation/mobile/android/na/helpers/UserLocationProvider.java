...
public class UserLocationProvider implements LocationProvider {
    ...
    public void getLocation(Context context, LocationCallback callback) {
        PersistenceProvider prefs = new PreferencePersistence("user_location");
        String latValue = (String) prefs.read(KEY_LAT, context);
        String lngValue = (String) prefs.read(KEY_LNG, context);
